// Kristofer Norman
// December 12th, 2021
//  This program is provided to read football statistics into a structure array, offers a 
//  menu to update statistics, prints data, and updates a file.
...
int main()
{
    ...
    do
    {
        ...
        case 3:
            ...
            cin >> numOfTouchDowns;
            cout << endl;
         updateTouchdowns(bigGiants, num, playerNum, numOfTouchDowns);
            break;
        case 4:
            cout << "Enter player name: ";
            cin >> name;
        ...
        case 5:
            ...
            cout << endl;
         updatePassingYards(bigGiants, numOfPassingYards, num, playerNum);
            break;
        case 6:
            cout << "Enter player name: ";
            cin >> name;
            ...
            cout << endl;
         UpdateReceivingYards(playerNum, bigGiants, numOfReceivingYards, num);
            break;
        case 7:
            cout << "Enter player name: ";
            cin >> name;
            ...
            cout << endl;
         RushYardUpdate(bigGiants, num, numOfRushingYards, playerNum);
            break;
        case 99:
            break;
        default:
...
int openFile(ifstream& in) {
    ...
    if (!in)
    {
        cout << filename << " input file does not exist. Program terminates!" <<
            endl;
        return 1;
    }
    return 0;
...
{
    howMany = 0;
    while ((!inf.eof() && howMany < MAX)) {
        inf >> list[howMany].name >> list[howMany].position >> list[howMany].touchDowns
            >> list[howMany].catches >> list[howMany].passingYards >> list[howMany].receivingYards
            >> list[howMany].rushingYards;
        howMany++;
     }
}
...
void saveData(ofstream& outF, footBallPlayerType list[], int length)
{
    ...
    if (!ret) {
        cout << "Output file did not open...data will not be output to a file. " <<
            endl;
        return;
    }
    for (int i = 0; i < length; i++)
...
void updateTouchdowns(footBallPlayerType list[], int length, int player, int update)
{
    if (player < 0 || player > length)
        cout << "Invalid player number." << endl << endl;
    else
    {
        list[player].touchDowns += update;
        cout << list[player].name << "'s touchdowns have been updated!" << endl << endl;
    }
}
...
void updatePassingYards(footBallPlayerType list[], int passingYards, int length, int playerNum)
{
    ...
    if (0 <= playerNum && playerNum < length)
    {
        oldData = list[playerNum].passingYards;
        list[playerNum].passingYards += passingYards;
        cout << list[playerNum].name << " passing yards is updated from " << oldData << " to " << list[playerNum].passingYards << "." << endl << endl;
    }
    else
    {
...
{
    int newRecievingYards;
    if (playerNum > num)// error checking for index
    {
        cout << "The index is out of bounds." << endl;
        return;
    }
    newRecievingYards = list[playerNum].receivingYards;
    newRecievingYards += numOfReceivingYards;
    list[playerNum].receivingYards = newRecievingYards;
...
void RushYardUpdate(footBallPlayerType list[], int length, int rushYards, int playerNum) {
    ...
    if (0 <= playerNum && playerNum < length)
    {
        OldNumbers = list[playerNum].rushingYards;
        list[playerNum].rushingYards = rushYards + list[playerNum].rushingYards;
        cout << list[playerNum].name << " Rushing yards is updated from " << OldNumbers << " to " << list[playerNum].rushingYards << "." << endl;
    }
